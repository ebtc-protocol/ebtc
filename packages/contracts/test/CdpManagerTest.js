...
contract('CdpManager', async accounts => {
  ...
  it("liquidate(): closes a Cdp that has ICR < MCR", async () => {
    ...
    th.assertIsApproximatelyEqual(ICR_Before.toString(), dec(4, 18), 100000000000000000)
    ...
    await priceFeed.setPrice('50080000000000000');
  ...
  it("liquidate(): decreases ActivePool ETH and EBTCDebt by correct amounts", async () => {
    ...
    // price drops to 1ETH:0,038EBTC, reducing Bob's ICR below MCR
    await priceFeed.setPrice('38000000000000000');
  ...
  it("liquidate(): increases DefaultPool ETH and EBTC debt by correct amounts", async () => {
    ...
    th.assertIsApproximatelyEqual(defaultPool_ETH_After.toString(), defaultPool_ETH.toString(), 10)
    th.assertIsApproximatelyEqual(defaultPool_RawEther_After.toString(), defaultPool_ETH.toString(), 10)
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    th.assertIsApproximatelyEqual(totalCollateralSnapshot_After, A_collateral.add(th.applyLiquidationFee(B_collateral)), 10)
  })
  ...
  it("liquidate(): updates the L_ETH and L_EBTCDebt reward-per-unit-staked totals", async () => {
    ...
    // price drops to 1ETH:0.038EBTC, reducing Carols's ICR below MCR
    await priceFeed.setPrice('38000000000000000');
    ...
    await priceFeed.setPrice('19000000000000000');
  ...
  it("liquidate(): Liquidates undercollateralized cdp if there are two cdps in the system", async () => {
    ...
    // Alice creates a single cdp with 0.7 ETH
    ...
    await stabilityPool.provideToSP(dec(1, 15), ZERO_ADDRESS, { from: alice })

    // Set ETH:EBTC price
    await priceFeed.setPrice('40000000000000000');
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Price drops
    await priceFeed.setPrice(dec(3698, 13))
    const price = await priceFeed.getPrice()
   ...
   (because his pending rewards bring his ICR < MCR) */
    ...
    // Check cdp statuses - A and B active (1), C liquidated (3)
  ...
  it('liquidateCdps(): liquidates a Cdp that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    assert.isFalse(await cdpManager.hasPendingRewards(_cCdpId))
  ...
  it('liquidateCdps(): closes every Cdp with ICR < MCR, when n > number of undercollateralized cdps', async () => {
    ...
    // Price drops to 1ETH:0.0xxEBTC, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(3000, 13))
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateCdps(): closes every cdp with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:0.03715EBTC, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(3715, 13))
    const price = await priceFeed.getPrice()
  ...
  it('redeemCollateral(): cancels the provided EBTC with debt from Cdps with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    ...
    const expectedTotalETHDrawn = redemptionAmount.mul(mv._1e18BN).div(toBN(dec(7428, 13))) // convert redemptionAmount EBTC to ETH, at ETH:USD price 200
