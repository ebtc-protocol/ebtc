...
contract('BorrowerOperations', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_LUSDDebt snapshots", async () => {
      ...
      let _colDefaulted = await troveManager.getTroveColl(carolIndex);
	  
      // liquidator get some debt asset to repay
      await openTrove({ extraLUSDAmount: toBN(dec(6000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: owner } })
      ...
      let _ethLiquidatorPre = await web3.eth.getBalance(owner);	  
      const tx = await troveManager.liquidate(carolIndex, { from: owner });	
      let _ethLiquidatorPost = await web3.eth.getBalance(owner);	
      const gasUsedETH = toBN(tx.receipt.effectiveGasPrice.toString()).mul(toBN((th.gasUsed(tx)).toString()));
      let _ethSeizedByLiquidator = toBN(_ethLiquidatorPost.toString()).sub(toBN(_ethLiquidatorPre.toString())).add(gasUsedETH);
      assert.equal(_ethSeizedByLiquidator.toString(), _colDefaulted.toString(), '!liquidator collateral balance');	 
      ...
      const alicePendingLUSDDebtReward = await troveManager.getPendingLUSDDebtReward(aliceIndex)
      const bobPendingLUSDDebtReward = await troveManager.getPendingLUSDDebtReward(bobIndex)
      for (reward of [alicePendingETHReward, bobPendingETHReward, alicePendingLUSDDebtReward, bobPendingLUSDDebtReward]) {
        assert.isTrue(reward.eq(toBN('0'))) // no redistribution over liquidation
      }
    ...
    it("addColl(), reverts if trove is non-existent or closed", async () => {
      ...
      // Bob gets liquidated
      await troveManager.liquidate(bobIndex, {from: owner})
    ...
    it("withdrawColl(): sends the correct amount of ETH to the user", async () => {
      ...
      let _withdrawTx = await borrowerOperations.withdrawColl(aliceIndex, dec(1, 'ether'), aliceIndex, aliceIndex, { from: alice, gasPrice: 10000000000 })
      let _gasEtherUsed = toBN(_withdrawTx.receipt.effectiveGasPrice.toString()).mul(toBN((th.gasUsed(_withdrawTx)).toString()));
      const alice_ETHBalance_After = toBN(web3.utils.toBN(await web3.eth.getBalance(alice)))
      const balanceDiff = alice_ETHBalance_After.sub(alice_ETHBalance_Before).add(_gasEtherUsed);
      console.log('_gasEtherUsed=' + _gasEtherUsed + ',alice_ETHBalance_After=' + alice_ETHBalance_After + ',alice_ETHBalance_Before=' + alice_ETHBalance_Before);
      assert.equal(balanceDiff.toString(), toBN(dec(1, 'ether')).toString());
    })
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_LUSDDebt snapshots", async () => {
      ...
      const pendingDebtReward_A = await troveManager.getPendingLUSDDebtReward(aliceIndex)
      const pendingCollReward_B = await troveManager.getPendingETHReward(bobIndex)
      const pendingDebtReward_B = await troveManager.getPendingLUSDDebtReward(bobIndex)
    ...
    it("withdrawLUSD(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.withdrawLUSD(aliceIndex, th._100pct, LUSDwithdrawal, aliceIndex, aliceIndex, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("withdrawLUSD(): decays a non-zero base rate", async () => {
      ...
      // D withdraws LUSD
      await borrowerOperations.withdrawLUSD(DIndex, th._100pct, dec(1, 18), AIndex, AIndex, { from: D })
      ...
      // E withdraws LUSD
      await borrowerOperations.withdrawLUSD(EIndex, th._100pct, dec(1, 18), AIndex, AIndex, { from: E })
    ...
    it("withdrawLUSD(): reverts if max fee > 100%", async () => {
      ...
      await assertRevert(borrowerOperations.withdrawLUSD(AIndex, dec(2, 18), dec(1, 18), AIndex, AIndex, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawLUSD(AIndex, '1000000000000000001', dec(1, 18), AIndex, AIndex, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })
    ...
    it("withdrawLUSD(): reverts if max fee < 0.5% in Normal mode", async () => {
      ...
      await assertRevert(borrowerOperations.withdrawLUSD(AIndex, 0, dec(1, 18), AIndex, AIndex, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawLUSD(AIndex, 1, dec(1, 18), AIndex, AIndex, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawLUSD(AIndex, '4999999999999999', dec(1, 18), AIndex, AIndex, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    xit("withdrawLUSD(): reverts if fee exceeds max fee percentage", async () => {
      ...
      await assertRevert(borrowerOperations.withdrawLUSD(AIndex, lessThan5pct, dec(3, 18), AIndex, AIndex, { from: A }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate() // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(borrowerOperations.withdrawLUSD(BIndex, dec(1, 16), dec(1, 18), AIndex, AIndex, { from: B }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(borrowerOperations.withdrawLUSD(CIndex, dec(3754, 13), dec(1, 18), AIndex, AIndex, { from: C }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 0.5%%
      await assertRevert(borrowerOperations.withdrawLUSD(DIndex, dec(5, 15), dec(1, 18), AIndex, AIndex, { from: D }), "Fee exceeded provided maximum")
    })

    xit("withdrawLUSD(): succeeds when fee is less than max fee percentage", async () => {
      await openTrove({ extraLUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLUSDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraLUSDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      const tx1 = await borrowerOperations.withdrawLUSD(AIndex, moreThan5pct, dec(1, 18), AIndex, AIndex, { from: A })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawLUSD(BIndex, dec(5, 16), dec(1, 18), AIndex, AIndex, { from: B })
      ...
      const tx3 = await borrowerOperations.withdrawLUSD(CIndex, dec(1, 17), dec(1, 18), AIndex, AIndex, { from: C })
      ...
      const tx4 = await borrowerOperations.withdrawLUSD(DIndex, dec(37659, 13), dec(1, 18), AIndex, AIndex, { from: D })
      assert.isTrue(tx4.receipt.status)

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.withdrawLUSD(EIndex, dec(1, 18), dec(1, 18), AIndex, AIndex, { from: E })
      assert.isTrue(tx5.receipt.status)
    })

    xit("withdrawLUSD(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      // D withdraws LUSD
      await borrowerOperations.withdrawLUSD(DIndex, th._100pct, dec(37, 18), AIndex, AIndex, { from: D })
      ...
      // E opens trove 
      await borrowerOperations.withdrawLUSD(EIndex, th._100pct, dec(12, 18), AIndex, AIndex, { from: E })
    ...
    it("withdrawLUSD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await borrowerOperations.withdrawLUSD(CIndex, th._100pct, dec(1, 18), CIndex, CIndex, { from: C })
    ...
    xit("withdrawLUSD(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    xit("withdrawLUSD(): borrowing at non-zero base rate sends LUSD fee to LQTY staking contract", async () => {
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      xit("withdrawLUSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        // time fast-forwards 1 year, and multisig stakes 1 LQTY
        await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
        await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
        await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    xit("withdrawLUSD(): Borrowing at non-zero base rate increases the LQTY staking contract LUSD fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    xit("withdrawLUSD(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    xit("withdrawLUSD(): Borrowing at zero base rate changes LUSD fees-per-unit-staked", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    xit("withdrawLUSD(): Borrowing at zero base rate sends debt request to user", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraLUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraLUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraLUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawLUSD(): reverts when calling address does not have active trove", async () => {
      ...
      const txBob = await borrowerOperations.withdrawLUSD(bobIndex, th._100pct, dec(100, 18), bobIndex, bobIndex, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to withdraw LUSD
      try {
        const txCarol = await borrowerOperations.withdrawLUSD(carolIndex, th._100pct, dec(100, 18), bobIndex, bobIndex, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawLUSD(): reverts when requested withdrawal amount is zero LUSD", async () => {
      ...
      const txBob = await borrowerOperations.withdrawLUSD(bobIndex, th._100pct, 1, bobIndex, bobIndex, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Alice attempts to withdraw 0 LUSD
      try {
        const txAlice = await borrowerOperations.withdrawLUSD(aliceIndex, th._100pct, 0, aliceIndex, aliceIndex, { from: alice })
    ...
    it("withdrawLUSD(): reverts when system is in Recovery Mode", async () => {
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawLUSD(aliceIndex, th._100pct, dec(100, 18), aliceIndex, aliceIndex, { from: alice })
      assert.isTrue(txAlice.receipt.status)
    ...
    it("withdrawLUSD(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      ...
      try {
        const txBob = await borrowerOperations.withdrawLUSD(bobIndex, th._100pct, dec(1, 18), bobIndex, bobIndex, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawLUSD(): reverts if system is in Recovery Mode", async () => {
      ...
      try {
        const txData = await borrowerOperations.withdrawLUSD(aliceIndex, th._100pct, '200', aliceIndex, aliceIndex, { from: alice })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
        assert.include(err.message, 'revert')
      }
    ...
    it("withdrawLUSD(): increases the Trove's LUSD debt by the correct amount", async () => {
      ...
      await borrowerOperations.withdrawLUSD(aliceIndex, th._100pct, await getNetBorrowingAmount(100), aliceIndex, aliceIndex, { from: alice })
    ...
    it("withdrawLUSD(): increases LUSD debt in ActivePool by correct amount", async () => {
      ...
      await borrowerOperations.withdrawLUSD(aliceIndex, th._100pct, await getNetBorrowingAmount(dec(10000, 18)), aliceIndex, aliceIndex, { from: alice })
    ...
    it("withdrawLUSD(): increases user LUSDToken balance by correct amount", async () => {
      ...
      await borrowerOperations.withdrawLUSD(aliceIndex, th._100pct, dec(10000, 18), aliceIndex, aliceIndex, { from: alice })
    ...
    it("repayLUSD(): reverts when repayment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.repayLUSD(aliceIndex, LUSDRepayment, aliceIndex, aliceIndex, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("repayLUSD(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2'))), A, A, { from: A, value: _colAmt })
      const AIndex = await sortedTroves.troveOfOwnerByIndex(A,0)

      const repayTxA = await borrowerOperations.repayLUSD(AIndex, 1, AIndex, AIndex, { from: A })
      assert.isTrue(repayTxA.receipt.status)

      let _debtAmt = dec(20, 21);
      let _repayAmt = dec(10, 21);
      await borrowerOperations.openTrove(th._100pct, _debtAmt, B, B, { from: B, value: _colAmt })
      const BIndex = await sortedTroves.troveOfOwnerByIndex(B,0)

      const repayTxB = await borrowerOperations.repayLUSD(BIndex, _repayAmt, BIndex, BIndex, { from: B })
    ...
    it("repayLUSD(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      ...
      const repayTxAPromise = borrowerOperations.repayLUSD(AIndex, 2, AIndex, AIndex, { from: A })
    ...
    xit("repayLUSD(): reverts when calling address does not own trove index supplied", async () => {
      await openTrove({ extraLUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      const txBob = await borrowerOperations.repayLUSD(bobIndex, dec(10, 18), bobIndex, bobIndex, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to repayLUSD
      try {
        const txCarol = await borrowerOperations.repayLUSD(bobIndex, dec(10, 18), bobIndex, bobIndex, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repayLUSD(): reverts when attempted repayment is > the debt of the trove", async () => {
      ...
      try {
        const txAlice = await borrowerOperations.repayLUSD(aliceIndex, aliceDebt.add(toBN(dec(1, 18))), aliceIndex, aliceIndex, { from: alice })
        assert.isFalse(txAlice.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repayLUSD(): reduces the Trove's LUSD debt by the correct amount", async () => {
      ...
      await borrowerOperations.repayLUSD(aliceIndex, aliceDebtBefore.div(toBN(10)), aliceIndex, aliceIndex, { from: alice })  // Repays 1/10 her debt
    ...
    //TODO: fix
    xit("repayLUSD(): can repay debt in Recovery Mode", async () => {
      await openTrove({ extraLUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      const tx = await borrowerOperations.repayLUSD(aliceIndex, aliceDebtBefore.div(toBN(10)), aliceIndex, aliceIndex, { from: alice })
    ...
    it("repayLUSD(): Reverts if borrower has insufficient LUSD balance to cover his debt repayment", async () => {
      ...
      const repayLUSDPromise_B = borrowerOperations.repayLUSD(BIndex, toBN(dec(6, 18)), BIndex, BIndex, { from: B })
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.adjustTrove(aliceIndex, th._100pct, 0, LUSDRepayment, false, aliceIndex, aliceIndex, { from: alice, value: collTopUp }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    xit("adjustTrove(): borrowing at non-zero base rate sends LUSD fee to LQTY staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    xit("adjustTrove(): Borrowing at non-zero base rate increases the LQTY staking contract LUSD fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 LQTY
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
      await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
      await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    it("adjustTrove(): Borrowing at zero base rate sends total requested LUSD to the user", async () => {
      ...
      // D adjusts trove
      const LUSDRequest_D = toBN(dec(40, 18))
      await borrowerOperations.adjustTrove(DIndex, th._100pct, 0, LUSDRequest_D, true, DIndex, DIndex, { from: D })
    ...
    it("adjustTrove(): reverts when LUSD repaid is > debt of the trove", async () => {
      ...
      const remainingDebt = (await troveManager.getTroveDebt(bobIndex)).sub(LUSD_GAS_COMPENSATION)
...
//        await assertRevert(txPromise_C, "BorrowerOps: Caller must be the borrower for a withdrawal")
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      // liquidator get some debt asset to repay
      await openTrove({ extraLUSDAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: owner } });
      ...
      const L_LUSDDebt_A_Snapshot = (await troveManager.rewardSnapshots(aliceIndex))[1]
      assert.isTrue(L_ETH_A_Snapshot.eq(toBN('0')))
      assert.isTrue(L_LUSDDebt_A_Snapshot.eq(toBN('0')))// no redistribution over liquidation
      ...
      const L_LUSDDebt_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(aliceIndex))[1]

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation.eq(toBN('0')))
      assert.isTrue(L_LUSDDebt_Snapshot_A_AfterLiquidation.eq(toBN('0')))
      ...
      const L_LUSDDebt_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(aliceIndex))[1]
    ...
    it("closeTrove(): applies pending rewards", async () => {
      // --- SETUP ---
      await bn8Signer.sendTransaction({ to: whale, value: ethers.utils.parseEther("40000")});
      ...
      let _ethLiquidatorPre = await web3.eth.getBalance(owner);	  
      let _debtLiquidatorPre = await lusdToken.balanceOf(owner);  
      const liquidationTx = await troveManager.liquidate(carolIndex, { from: owner }); 
      let _ethLiquidatorPost = await web3.eth.getBalance(owner);	  
      let _debtLiquidatorPost = await lusdToken.balanceOf(owner);
      let _debtDecreased = toBN(_debtLiquidatorPre.toString()).sub(toBN(_debtLiquidatorPost.toString()));
      const gasUsedETH = toBN(liquidationTx.receipt.effectiveGasPrice.toString()).mul(toBN((th.gasUsed(liquidationTx)).toString()));
      let _ethSeizedByLiquidator = toBN(_ethLiquidatorPost.toString()).sub(toBN(_ethLiquidatorPre.toString())).add(gasUsedETH);
      ...
      // Carol's liquidated coll (1 ETH) and drawn debt should have sent to and burnt from liquidator
      assert.isAtMost(th.getDifference(_ethSeizedByLiquidator, liquidatedColl_C), 100)
      assert.isAtMost(th.getDifference(_debtDecreased, liquidatedDebt_C), 100)

      const pendingCollReward_A = await troveManager.getPendingETHReward(aliceIndex)
      const pendingDebtReward_A = await troveManager.getPendingLUSDDebtReward(aliceIndex)
      assert.equal(pendingCollReward_A.toString(), '0')
      assert.equal(pendingDebtReward_A.toString(), '0') // no redistribution over liquidation
    ...
    it("openTrove(): Opens a trove with net debt >= minimum net debt", async () => {
      ...
      let _colAmt = dec(1000, 18);
      const txA = await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(1))), th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: A, value: _colAmt })
      assert.isTrue(txA.receipt.status)
      const AIndex = await sortedTroves.troveOfOwnerByIndex(A,0)
      assert.isTrue(await sortedTroves.contains(AIndex))

      let _debtAmt = toBN(dec(47789898,14));
      const txC = await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(_debtAmt)), th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: C, value: _colAmt })
    ...
    it("openTrove(): reverts if net debt < minimum net debt", async () => {
      let _colAmt = dec(1000, 18);
      const txAPromise = borrowerOperations.openTrove(th._100pct, 0, th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: A, value: _colAmt })
      await assertRevert(txAPromise, "revert")

      const txBPromise = borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.sub(toBN(1))), th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: B, value: _colAmt })
      await assertRevert(txBPromise, "revert")

      const txCPromise = borrowerOperations.openTrove(th._100pct, MIN_NET_DEBT.sub(toBN(dec(173, 18))), th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: C, value: _colAmt })
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, LUSDRequest_D, th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: D, value: dec(500, 'ether') })
    ...
    it("openTrove(): account can open multiple troves", async () => {
      ...
      await openTrove({ extraLUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
    })
    ...
    it("openTrove(): records up-to-date initial snapshots of L_ETH and L_LUSDDebt", async () => {
      ...
      // liquidator get some debt asset to repay
      await openTrove({ extraLUSDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: owner } });
      ...
      assert.isTrue(L_LUSD.eq(toBN('0')))// no redistribution over liquidation
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        await borrowerOperations.openTrove(th._100pct, troveLUSDAmount, th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: alice, value: troveColl })
        await borrowerOperations.openTrove(th._100pct, troveLUSDAmount, th.DUMMY_BYTES32, th.DUMMY_BYTES32, { from: bob, value: troveColl })
	  
        // liquidator get some debt asset to repay
        await lusdToken.transfer(owner, (await lusdToken.balanceOf(alice)), { from: alice});
        await lusdToken.transfer(owner, (await lusdToken.balanceOf(bob)), { from: bob});
        ...
        const liquidationTx = await troveManager.liquidate(bobIndex, {from: owner})
        assert.isFalse(await sortedTroves.contains(bobIndex))
        ...
        const expectedTCR = (troveColl.add(toBN('0'))).mul(price)
          .div(troveTotalDebt.add(toBN('0')))
      ...
      it("collChange = 0, debtChange is positive", async () => {
        ...
        const expectedTCR = (troveColl.add(toBN('0'))).mul(price)
          .div(troveTotalDebt.add(toBN('0')).add(toBN(debtChange)))
      ...
      it("collChange = 0, debtChange is negative", async () => {
        ...
        const expectedTCR = (troveColl.add(toBN('0'))).mul(price)
          .div(troveTotalDebt.add(toBN('0')).sub(toBN(dec(100, 18))))
      ...
      it("collChange is positive, debtChange is 0", async () => {
        ...
        const expectedTCR = (troveColl.add(toBN('0')).add(toBN(collChange))).mul(price)
          .div(troveTotalDebt.add(toBN('0')))
      ...
      it("collChange is negative, debtChange is 0", async () => {
        ...
        const expectedTCR = (troveColl.add(toBN('0')).sub(toBN(dec(1, 'ether')))).mul(price)
          .div(troveTotalDebt.add(toBN('0')))
      ...
      it("collChange is negative, debtChange is negative", async () => {
        ...
        // liquidator get some debt asset to repay
        await openTrove({ extraLUSDAmount: toBN(dec(200000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: owner } });
      ...
      it("collChange is positive, debtChange is negative", async () => {
        ...
        const expectedTCR = (troveColl.add(toBN('0')).add(toBN(dec(1, 'ether')))).mul(price)
          .div(troveTotalDebt.add(toBN('0')).sub(toBN(dec(100, 18))))
      ...
      it("collChange is negative, debtChange is positive", async () => {
        ...
        const newTCR = (await borrowerOperations.getNewTCRFromTroveChange(collChange, false, debtChange, true, price))
        let _debtSystem = await troveManager.getEntireSystemDebt();
        let _colSystem = await troveManager.getEntireSystemColl();

        const expectedTCR = (troveColl.add(toBN('0')).sub(toBN(collChange))).mul(price)
          .div(troveTotalDebt.add(toBN('0')).add(toBN(debtChange)))
    ...
    if (!withProxy) {
      it("closeTrove(): fails if owner cannot receive ETH", async () => {
        ...
        await bn8Signer.sendTransaction({ to: owner, value: ethers.utils.parseEther("1000")});
        await nonPayable.forward(borrowerOperations.address, openTroveData, { from: owner, value: dec(10000, 'ether') })
