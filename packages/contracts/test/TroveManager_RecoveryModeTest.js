...
contract('TroveManager - in Recovery Mode', async accounts => {
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    ...
    const { lusdAmount: A_lusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraEBTCAmount: dec(300, 18), extraParams: { from: alice } })
    const { lusdAmount: B_lusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraEBTCAmount: dec(200, 18), extraParams: { from: bob } })
    const { lusdAmount: C_lusdAmount } = await openTrove({ ICR: toBN(dec(206, 16)), extraEBTCAmount: dec(100, 18), extraParams: { from: carol } })
    let _aliceTroveId = await sortedTroves.troveOfOwnerByIndex(alice, 0);
    let _bobTroveId = await sortedTroves.troveOfOwnerByIndex(bob, 0);
    let _carolTroveId = await sortedTroves.troveOfOwnerByIndex(carol, 0);
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    ...
    const { lusdAmount: W_lusdAmount } = await openTrove({ ICR: toBN(dec(300, 16)), extraEBTCAmount: dec(4000, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(W_lusdAmount, ZERO_ADDRESS, { from: whale })

    const { lusdAmount: A_lusdAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(191, 16)), extraEBTCAmount: dec(40, 18), extraParams: { from: alice } })
    const { lusdAmount: B_lusdAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraEBTCAmount: dec(240, 18), extraParams: { from: bob } })
