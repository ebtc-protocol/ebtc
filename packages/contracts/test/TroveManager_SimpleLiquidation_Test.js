...
contract('TroveManager - Simple Liquidation without Stability Pool', async accounts => {
  ...
  it("Troves below MCR will be liquidated", async () => {
      ...
      assert.equal(liquidationEvents[0].args[1].toString(), _debtBorrowed.toString(), '!liquidated trove debt');
      assert.equal(liquidationEvents[0].args[2].toString(), _colDeposited.toString(), '!liquidated trove collateral');
