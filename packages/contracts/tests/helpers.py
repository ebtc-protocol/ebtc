...
def logGlobalState(contracts):
    ...
    activePoolColl = contracts.activePool.getSystemCollShares()
    activePoolDebt = contracts.activePool.getSystemDebt()
    total_debt = (activePoolDebt).to("ether")
    total_coll = (activePoolColl).to("ether")
    print('Total Debt      ', total_debt)
    print('Total Coll      ', total_coll)
    SP_EBTC = 0 ## Stability Pool is gone.to("ether")
    SP_ETH =  0 ## Stability Pool is gone.to("ether")
