...
def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current,
                     price_LUSD, price_LQTY_current, data, index):
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            tx = contracts.troveManager.liquidateTroves(NUM_LIQUIDATIONS,
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current,
                  index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        current_icr = contracts.troveManager.getCurrentICR(account,
        ...
        check = (current_icr - working_trove['CR_initial']) / (
                working_trove['CR_initial'] * working_trove['Rational_inattention'])
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                if pending == 0:
                    contracts.borrowerOperations.repayLUSD(repay_amount, hints[0], hints[1],
                                                           {'from': account})
                ...
                if is_new_tcr_above_ccr(contracts, 0, False, withdraw_amount_wei, True,
                                        floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawLUSD(MAX_FEE, withdraw_amount_wei,
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove,
    ...
    debt_change = floatToWei(supply_trove) + LUSD_GAS_COMPENSATION
...
---
With the supply and demand of LUSD tokens defined above, the price of LUSD at the current period 
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    ...
    try:
        ...
        print(
            f"Redemption rate: "
            f"{contracts.troveManager.getRedemptionRateWithDecay() * 100 / 1e18} %")
