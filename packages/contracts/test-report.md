...
$ hardhat test
  ...
  Contract: BorrowerOperations
    Without proxy
      ...
      ✓ withdrawEBTC(): reverts if max fee < 0.5% in Normal mode (639ms)
      ✓ withdrawEBTC(): reverts if fee exceeds max fee percentage (938ms)
      ✓ withdrawEBTC(): succeeds when fee is less than max fee percentage (1272ms)
      ✓ withdrawEBTC(): doesn't change base rate if it is already zero (1044ms)
      ...
      ✓ withdrawEBTC(): Borrowing at zero base rate sends debt request to user (790ms)
      ✓ withdrawEBTC(): reverts when calling address does not have active cdp (370ms)
      ✓ withdrawEBTC(): reverts when requested withdrawal amount is zero EBTC (459ms)
      ✓ withdrawEBTC(): reverts when system is in Recovery Mode (649ms)
...
gasUsed:  636956
  ...
  Contract: CdpManager
    ...
    ✓ redeemCollateral(): a redemption made at zero base rate send a non-zero stEthRedemptionFee to LQTY staking contract (5136ms)
    ✓ redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-LQTY-staked in LQTY Staking contract (1207ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate send a non-zero stEthRedemptionFee to LQTY staking contract (1730ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-LQTY-staked in the staking contract (5971ms)
    ✓ redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - stEthRedemptionFee) to the redeemer (1113ms)
    ...
    ✓ getPendingDebtRedistribution(): Returns 0 if there is no pending EBTCDebt reward (618ms)
    ...
    ✓ hasPendingDebtRedistribution(): Returns false it cdp is not active
