...
contract('Echidna debugger', async accounts => {
  ...
  it.skip('cdp order', async () => {
    ...
    const icr1_before = await cdpManager.getICR(cdp1, '1000000000000000000')
    const icr2_before = await cdpManager.getICR(cdp2, '1000000000000000000')
    ...
    const icr1_after = await cdpManager.getICR(cdp1, '1000000000000000000')
    const icr2_after = await cdpManager.getICR(cdp2, '1000000000000000000')
    console.log('Cdp 1', icr1_after, icr1_after.toString())
    console.log('Cdp 2', icr2_after, icr2_after.toString())

    const icr1_after_price = await cdpManager.getICR(cdp1, '78051143795343077331468494330613608802436946862454908477491916')
    const icr2_after_price = await cdpManager.getICR(cdp2, '78051143795343077331468494330613608802436946862454908477491916')
