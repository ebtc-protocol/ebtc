...
class TestHelper {
  ...
  static async getCdpEntireColl(contracts, cdp) {
    return this.toBN((await contracts.cdpManager.getVirtualDebtAndCollShares(cdp))[1])
  }

  static async getCdpEntireDebt(contracts, cdp) {
    return this.toBN((await contracts.cdpManager.getVirtualDebtAndCollShares(cdp))[0])
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        ...
        const totalStEthToSend = redemptionTx.logs[i].args[2]
        const stEthRedemptionFee = redemptionTx.logs[i].args[3]

        return [EBTCAmount, totalEBTCRedeemed, totalStEthToSend, stEthRedemptionFee]
      }
    }
  ...
  static async withdrawEBTC(contracts, {
    ...
    if (ICR) {
      ...
      const { debt, coll } = await contracts.cdpManager.getVirtualDebtAndCollShares(_cdpId)
      const price = await contracts.priceFeedTestnet.getPrice()
