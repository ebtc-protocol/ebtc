...
class DeploymentHelper {
  ...
  static async deployViaCreate3(ebtcDeployer, _argTypes, _argValues, _code, _salt) {
    ...
    if (_argTypes.length > 0 && _argValues.length > 0) {
      ...
      _deployTx = await ebtcDeployer.deployWithCreationCodeAndConstructorArgs(
        _salt,
        _code,
        _arg,
        _txParams
    ...
    if (DeploymentHelper.deployWait > 0) {
      console.log(
        _salt +
          "_deployedTx=" +
          DeploymentHelper.getTxHash(_deployTx) +
          ":wait deploy complete..." +
          DeploymentHelper.deployWait / 1000 +
          " seconds"
      );
      await waitFunction();
  ...
  static async deployGovernor(ebtcDeployer, _expectedAddr, ownerAddress) {
    ...
    const _deployedAddr = await this.deployViaCreate3(
      ebtcDeployer,
      _argTypes,
      _argValues,
      _code,
  ...
  static async deployPriceFeed(ebtcDeployer, _expectedAddr, _collEthCLFeed, _ethBtcCLFeed) {
    ...
    const _argValues = [
      ...
      _collEthCLFeed,
      _ethBtcCLFeed
    ]; // use address(0) for IFallbackCaller
  ...
  static async deployCollateralTestnet() {
    ...
    const collateral = await CollateralTokenTester.new({
      gasPrice: DeploymentHelper.deployGasPrice * 2,
      nonce: _nonce
    });
    return collateral;
  }
  ...
  static async deployChainLinkStethUSDTestnet(ebtcDeployer) {
    const contractFactory = await ethers.getContractFactory(
      "TestContracts/testnet/ChainLinkStethUSDTestnet",
      (
        await ethers.getSigners()
      )[0]
    );

    const deployedContract = await contractFactory.deploy(ebtcDeployer.address);
  ...
  */
  static async deployLiquityCoreHardhat() {
    ...
    const authority = await DeploymentHelper.deployGovernor(
      ebtcDeployer,
      _expectedAddr,
      accounts[0]
    );

    const liquidationLibrary = await DeploymentHelper.deployLiquidationLibrary(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );

    const cdpManager = await DeploymentHelper.deployCdpManager(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );
    const borrowerOperations = await DeploymentHelper.deployBorrowerOperations(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );
    ...
    const priceFeedTestnet = await DeploymentHelper.deployPriceFeedTestnet(
      ebtcDeployer,
      _expectedAddr
    );

    const activePool = await DeploymentHelper.deployActivePool(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );
    const collSurplusPool = await DeploymentHelper.deployCollSurplusPool(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );
    ...
    const hintHelpers = await DeploymentHelper.deployHintHelper(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );

    const feeRecipient = await DeploymentHelper.deployFeeRecipient(
      ebtcDeployer,
      _expectedAddr,
      accounts[0]
    );
  ...
  */
  static async deployTesterContractsHardhat() {
    ...
    testerContracts.authority = await DeploymentHelper.deployGovernor(
      ebtcDeployer,
      _expectedAddr,
      accounts[0]
    );
    testerContracts.liquidationLibrary = await DeploymentHelper.deployLiquidationLibrary(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );

    testerContracts.cdpManager = await DeploymentHelper.deployCdpManagerTester(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );
    testerContracts.borrowerOperations = await DeploymentHelper.deployBorrowerOperationsTester(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );
    testerContracts.ebtcToken = await DeploymentHelper.deployEBTCTokenTester(
      ebtcDeployer,
      _expectedAddr
    );

    testerContracts.priceFeedTestnet = await DeploymentHelper.deployPriceFeedTestnet(
      ebtcDeployer,
      _expectedAddr
    );

    testerContracts.activePool = await DeploymentHelper.deployActivePoolTester(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );
    testerContracts.collSurplusPool = await DeploymentHelper.deployCollSurplusPool(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );

    testerContracts.sortedCdps = await DeploymentHelper.deploySortedCdps(
      ebtcDeployer,
      _expectedAddr
    );
    testerContracts.hintHelpers = await DeploymentHelper.deployHintHelper(
      ebtcDeployer,
      _expectedAddr,
      collateral.address
    );

    testerContracts.feeRecipient = await DeploymentHelper.deployFeeRecipient(
      ebtcDeployer,
      _expectedAddr,
      accounts[0]
    );
  ...
  static async deployProxyScripts(contracts, LQTYContracts, owner, users) {
    ...
    contracts.cdpManager = new CdpManagerProxy(
      owner,
      proxies,
      cdpManagerScript.address,
    ...
    contracts.ebtcToken = new TokenProxy(
      owner,
      proxies,
      ebtcTokenScript.address,
      contracts.ebtcToken
    );

    const lqtyStakingScript = await LQTYStakingScript.new(LQTYContracts.feeRecipient.address);
    LQTYContracts.feeRecipient = new LQTYStakingProxy(
      owner,
      proxies,
      lqtyStakingScript.address,
      LQTYContracts.feeRecipient
    );
  }
