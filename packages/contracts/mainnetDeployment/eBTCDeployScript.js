...
const CHAINLINK_STETH_USD_TESTNET_NAME = "chainLinkStethUSDTestnet";
...
class EBTCDeployerScript {
  constructor(useMockCollateral, useMockPriceFeed, mainnetDeploymentHelper, deploymentState, configParams, deployerWallet) {
    ...
    this.authorityOwner = checkValidItem(configParams.externalAddress["authorityOwner"])
      ? configParams.externalAddress["authorityOwner"]
      : deployerWallet.address;
    this.feeRecipientOwner = checkValidItem(configParams.externalAddress["feeRecipientOwner"])
      ? configParams.externalAddress["feeRecipientOwner"]
      : deployerWallet.address;

    this.collEthCLFeed = checkValidItem(configParams.externalAddress["collEthCLFeed"])
      ? configParams.externalAddress["collEthCLFeed"]
      : deployerWallet.address;
    this.ethBtcCLFeed = checkValidItem(configParams.externalAddress["ethBtcCLFeed"])
      ? configParams.externalAddress["ethBtcCLFeed"]
      : deployerWallet.address;
  }
  ...
  async deployStateViaHelper(_stateName, _expectedAddr) {
    ...
    if (_stateName == GOVERNOR_STATE_NAME) {
      ...
      _deployedState = useMockPriceFeed
        ? await DeploymentHelper.deployPriceFeedTestnet(ebtcDeployer, _expectedAddr)
    ...
    } else if (_stateName == CHAINLINK_STETH_USD_TESTNET_NAME) {
      _deployedState = await DeploymentHelper.deployChainLinkStethUSDTestnet(ebtcDeployer);
    }
  ...
  async loadDeployedState(_stateName) {
    ...
    if (_stateName == GOVERNOR_STATE_NAME) {
      ...
      _deployedState = await (await ethers.getContractFactory("ChainLinkStethUSDTestnet")).attach(deploymentState[_stateName]["address"]);
      console.log("Sanity checking: chainLinkStethUSDTestnet.latestAnswer()=" + (await _deployedState.latestAnswer()));
    }
    return _deployedState;
  }
  ...
  async loadOrDeployCollateral() {
    ...
    if (_checkExistDeployment["_toDeploy"] && this.useMockCollateral) {
      // mock collateral to be deployed
      _collateral = await this.deployStateViaHelper(COLLATERAL_STATE_NAME, "");
    } else if (this.useMockCollateral) {
      // mock collateral already deployed
  ...
  async eBTCDeployCore() {
    ...
    _constructorArgs = this.useMockPriceFeed
      ? [_expectedAddr[0]]
      : [ethers.constants.AddressZero, _expectedAddr[0], this.collEthCLFeed, this.ethBtcCLFeed];
    ...
    // deploy chainLinkStethUSDTestnet
    console.log(chalk.cyan("[ChainLinkStethUSDTestnet]"));
    _constructorArgs = [_expectedAddr[1]];
    let chainLinkStethUSDTestnet = await this.deployOrLoadState(CHAINLINK_STETH_USD_TESTNET_NAME, _expectedAddr, _constructorArgs);
...
async function main() {
  ...
  console.log(
    "deploy with " + (useMockCollateral ? "mock collateral & " : " existing collateral & ") + (useMockPriceFeed ? "mock feed" : "original feed")
  );
