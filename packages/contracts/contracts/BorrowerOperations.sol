...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    event CdpUpdated(bytes32 indexed _cdpId, address indexed _borrower, uint _debt, uint _coll, uint stake, BorrowerOperation operation);
    ...
    function openCdp(uint _maxFeePercentage, uint _EBTCAmount, bytes32 _upperHint, bytes32 _lowerHint) external payable override {
...
//        _requireCdpisNotActive(contractsCache.cdpManager, msg.sender);
        ...
        if (isRecoveryMode) {
            ...
            uint newTCR = _getNewTCRFromCdpChange(msg.value, true, vars.compositeDebt, true, vars.price);  // bools: coll increase, debt increase
        ...
        emit CdpUpdated(_cdpId, msg.sender, vars.compositeDebt, msg.value, vars.stake, BorrowerOperation.openCdp);
    ...
    function moveETHGainToCdp(bytes32 _cdpId, bytes32 _upperHint, bytes32 _lowerHint) external payable override {
    ...
    function adjustCdp(bytes32 _cdpId, uint _maxFeePercentage, uint _collWithdrawal, uint _EBTCChange, bool _isDebtIncrease, bytes32 _upperHint, bytes32 _lowerHint) external payable override {
        _adjustCdp(_cdpId, _collWithdrawal, _EBTCChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFeePercentage);
    }
	
    function _adjustCdp(bytes32 _cdpId, uint _collWithdrawal, uint _EBTCChange, bool _isDebtIncrease, bytes32 _upperHint, bytes32 _lowerHint, uint _maxFeePercentage) internal {
        _requireCdpOwner(_cdpId);	
        _adjustCdpInternal(_cdpId, _collWithdrawal, _EBTCChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFeePercentage);
    }

    /*
    * _adjustCdp(): Alongside a debt change, this function can perform either a collateral top-up or a collateral withdrawal. 
    *
    * It therefore expects either a positive msg.value, or a positive _collWithdrawal argument.
    *
    * If both are positive, it will revert.
    */
    function _adjustCdpInternal(bytes32 _cdpId, uint _collWithdrawal, uint _EBTCChange, bool _isDebtIncrease, bytes32 _upperHint, bytes32 _lowerHint, uint _maxFeePercentage) internal {
        ...
        vars.newICR = _getNewICRFromCdpChange(vars.coll, vars.debt, vars.collChange, vars.isCollIncrease, vars.netDebtChange, _isDebtIncrease, vars.price);
        ...
        (vars.newColl, vars.newDebt) = _updateCdpFromAdjustment(contractsCache.cdpManager, _cdpId, vars.collChange, vars.isCollIncrease, vars.netDebtChange, _isDebtIncrease);
        vars.stake = contractsCache.cdpManager.updateStakeAndTotalStakes(_cdpId);

        // Re-insert cdp in to the sorted list
        uint newNICR = _getNewNominalICRFromCdpChange(vars.coll, vars.debt, vars.collChange, vars.isCollIncrease, vars.netDebtChange, _isDebtIncrease);
        sortedCdps.reInsert(_cdpId, newNICR, _upperHint, _lowerHint);

        emit CdpUpdated(_cdpId, _borrower, vars.newDebt, vars.newColl, vars.stake, BorrowerOperation.adjustCdp);
        emit EBTCBorrowingFeePaid(_cdpId,  vars.EBTCFee);
    ...
    function _triggerBorrowingFee(ICdpManager _cdpManager, IEBTCToken _ebtcToken, uint _EBTCAmount, uint _maxFeePercentage) internal returns (uint) {
    ...
    (
        ...
        returns (uint, uint)
    {
        uint newColl = (_isCollIncrease) ? _cdpManager.increaseCdpColl(_cdpId, _collChange)
                                        : _cdpManager.decreaseCdpColl(_cdpId, _collChange);
        uint newDebt = (_isDebtIncrease) ? _cdpManager.increaseCdpDebt(_cdpId, _debtChange)
...
//    }
        ...
        view 
    {
        ...
        */
        if (_isRecoveryMode) {
            ...
            _vars.newTCR = _getNewTCRFromCdpChange(_vars.collChange, _vars.isCollIncrease, _vars.netDebtChange, _isDebtIncrease, _vars.price);
    ...
    function _requireValidEBTCRepayment(uint _currentDebt, uint _debtRepayment) internal pure {
        require(_debtRepayment <= _currentDebt.sub(EBTC_GAS_COMPENSATION), "BorrowerOps: Amount repaid must not be larger than the Cdp's debt");
    }
    ...
    (
        ...
        returns (uint)
    {
        (uint newColl, uint newDebt) = _getNewCdpAmounts(_coll, _debt, _collChange, _isCollIncrease, _debtChange, _isDebtIncrease);
