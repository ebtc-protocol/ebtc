...
interface IBorrowerOperations{
    function openCdp(uint _maxFeePercentage, uint _EBTCAmount, bytes32 _upperHint, bytes32 _lowerHint) external payable;
}
...
contract MultipleCdpsTester {
    ...
    function openCdps(uint256 _count, uint _maxFeePercentage, uint _EBTCAmount, bytes32 _upperHint, bytes32 _lowerHint) external payable{
        uint256 _idx;
        for (;_idx < _count;){
             uint256 _singleCol = msg.value / _count;
             require(_singleCol > 0, '!msgVal');
             borrowerOperations.openCdp{value: _singleCol}(_maxFeePercentage, _EBTCAmount, _upperHint, _lowerHint);
