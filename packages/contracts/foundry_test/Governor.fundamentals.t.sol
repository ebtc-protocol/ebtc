...
contract GovernorFundamentalsTest is eBTCBaseFixture {
    ...
    function test_GetUsersByRoleReturnsExpected() public {
        // address[] memory expectedUsers = new address[](2);
        // expectedUsers[0] = address(0x1);
        // expectedUsers[1] = address(0x2);
        // authority.setRole(expectedUsers[0], 1);
        // authority.setRole(expectedUsers[1], 1);
        // address[] memory users = authority.getUsersByRole(1);
        // assertEq(users.length, expectedUsers.length, "Returned users length mismatch");
        // for (uint i = 0; i < users.length; i++) {
        //     assertEq(users[i], expectedUsers[i], "Returned user address mismatch");
        // }
    }
    ...
    function test_GetUsersByRoleReturnsExpectedAfterAddingAndRemovingUsers() public {
        // address[] memory expectedUsers = new address[](1);
        // expectedUsers[0] = address(0x1);
        // authority.setRole(expectedUsers[0], 1);
        // authority.setRole(address(0x2), 1);
        // authority.setRole(address(0x2), 0); // Remove user
        // address[] memory users = authority.getUsersByRole(1);
        // assertEq(users.length, expectedUsers.length, "Returned users length mismatch");
        // for (uint i = 0; i < users.length; i++) {
        //     assertEq(users[i], expectedUsers[i], "Returned user address mismatch");
