    ...
    IERC3156FlashBorrower
{
    ...
    function testGeneral08ForTarget05Again() public {
        bytes32 _cdp1 = openCdp(
            67534042799335353648407647554112468697195277953615236438520200454730440793371,
            8
        );
        ...
        liquidate(5498870406856203425250123995524908869201088216038261097215333899055942143363);
        console2.log(
            "lastEBTCDebtErrorRedistribution:",
            cdpManager.lastEBTCDebtErrorRedistribution()
        );

        uint256 _cdp1Coll = cdpManager.getSyncedCdpCollShares(_cdp1);
        uint256 _cdp2Coll = cdpManager.getSyncedCdpCollShares(_cdp2);
        uint256 _cdp1Debt = cdpManager.getSyncedCdpDebt(_cdp1);
        uint256 _cdp2Debt = cdpManager.getSyncedCdpDebt(_cdp2);
        uint256 _sysColl = cdpManager.getSyncedSystemColl();
        uint256 _sysDebt = activePool.getSystemDebt();
        console2.log("_cdp1Coll:", _cdp1Coll);
        console2.log("_cdp2Coll:", _cdp2Coll);
        console2.log("_cdp1Debt:", _cdp1Debt);
        console2.log("_cdp2Debt:", _cdp2Debt);
        console2.log("_sysColl:", _sysColl);
        console2.log("_sysDebt:", _sysDebt);
        console2.log("_tcr:", cdpManager.getSyncedTCR(priceFeedMock.fetchPrice()));

        bytes32 _cdp3 = openCdp(
            65081459863949768631643321477651888220057877973928177303332830234177197138896,
            27
        );
        closeCdp(115792089237316195423570985008687907853269984665640564039457584007913129639680);

        _cdp1Coll = cdpManager.getSyncedCdpCollShares(_cdp1);
        _cdp2Coll = cdpManager.getSyncedCdpCollShares(_cdp2);
        uint256 _cdp3Coll = cdpManager.getSyncedCdpCollShares(_cdp3);
        _cdp1Debt = cdpManager.getSyncedCdpDebt(_cdp1);
        _cdp2Debt = cdpManager.getSyncedCdpDebt(_cdp2);
        uint256 _cdp3Debt = cdpManager.getSyncedCdpDebt(_cdp3);
        _sysColl = cdpManager.getSyncedSystemColl();
        _sysDebt = activePool.getSystemDebt();
        console2.log("_cdp1Coll:", _cdp1Coll);
        console2.log("_cdp2Coll:", _cdp2Coll);
        console2.log("_cdp3Coll:", _cdp3Coll);
        console2.log("_cdp1Debt:", _cdp1Debt);
        console2.log("_cdp2Debt:", _cdp2Debt);
        console2.log("_cdp3Debt:", _cdp3Debt);
        console2.log("_sysColl:", _sysColl);
        console2.log("_sysDebt:", _sysDebt);
        console2.log("_tcr:", cdpManager.getSyncedTCR(priceFeedMock.fetchPrice()));
