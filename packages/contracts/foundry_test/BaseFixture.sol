...
contract eBTCBaseFixture is Test, BytecodeReader {
    ...
    */
    ...
    function _verifyCdpOrderingInvariant_NICR() internal returns (bool) {
        bytes32 lastId = sortedCdps.getLast();
        bytes32 firstId = sortedCdps.getFirst();
        bytes32 cdpId = lastId;
        ...
        uint256 lastICR = cdpManager.getNominalICR(lastId);
        uint256 firstICR = cdpManager.getNominalICR(firstId);

        // check if head has the largest ICR and tail has the smallest
        if (lastICR > firstICR) {
            return false;
        }

        // traverse the list from tail to head, checking if the ICR of each node is no larger than its prev
        while (cdpId != firstId) {
            uint256 currentICR = cdpManager.getNominalICR(cdpId);
            bytes32 prevId = sortedCdps.getPrev(cdpId);
            uint256 prevICR = cdpManager.getNominalICR(prevId);

            if (currentICR > prevICR) {
                return false;
            }
    ...
    function _verifyCdpOrderingInvariant_ICR() internal returns (bool) {
        bytes32 lastId = sortedCdps.getLast();
        bytes32 firstId = sortedCdps.getFirst();
        bytes32 cdpId = lastId;
        ...
        uint256 lastICR = cdpManager.getCurrentICR(lastId, price);
        uint256 firstICR = cdpManager.getCurrentICR(firstId, price);

        console.log("lastICR", lastICR);
        console.log("firstICR", firstICR);
        ...
        // traverse the list from tail to head, checking if the ICR of each node is no larger than its prev
        while (cdpId != firstId) {
            uint256 currentICR = cdpManager.getCurrentICR(cdpId, price);
            console.log("ICR", currentICR);
            bytes32 prevId = sortedCdps.getPrev(cdpId);
            uint256 prevICR = cdpManager.getCurrentICR(prevId, price);
